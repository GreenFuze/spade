...
STEP 2: Use 'list_dir' with glob patterns to efficiently explore (e.g., "*.cpp", "*.java", "*.py")
STEP 3: Read only essential files (build configs, main source files, not all files)
STEP 4: After exploring a directory, move to the next one and forget the previous context
STEP 5: Repeat for all directories
STEP 6: Provide final summary JSON with all findings
...
- MANDATORY: Use glob patterns to efficiently filter files by type (e.g., "*.cpp", "*.java", "*.py")
- MANDATORY: Read build configuration files in each directory to understand build structure
- MANDATORY: Focus on essential files only - don't read every single file
- MANDATORY: After exploring each directory, move to the next one and forget the previous context
...
- Explore ONE directory at a time
- Use glob patterns to filter files efficiently
- Don't read every file - focus on build configs and main source files
- After exploring a directory, move to the next one and forget the previous context
- This prevents context explosion in large repositories

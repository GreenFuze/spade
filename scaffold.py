...
from schemas import NodeUpdate, HighLevelComponent
from schemas import save_json, save_json_data
...
class ScaffoldStore:
    ...
    def __init__(self, workspace):
        ...
        self.nodes_path = Path("repository_scaffold.json")
        self.components_path = Path("high_level_components.json")

        # Initialize files if they don't exist
        try:
            with self.workspace.open_scaffold_file(
                "repository_scaffold.json", "r"
            ) as f:
                pass  # File exists
        except FileNotFoundError:
            with self.workspace.open_scaffold_file(
                "repository_scaffold.json", "w"
            ) as f:
                json.dump({}, f)  # dict[path] -> stored node dict

        try:
            with self.workspace.open_scaffold_file(
                "high_level_components.json", "r"
            ) as f:
                pass  # File exists
        except FileNotFoundError:
            with self.workspace.open_scaffold_file(
                "high_level_components.json", "w"
            ) as f:
                json.dump([], f)  # list of components
    ...
    def load_nodes(self) -> Dict:
        ...
        try:
            ...
            get_logger().warning(f"[scaffold] failed to load nodes: {e}; using empty")
            return {}
    ...
    def save_nodes(self, data: Dict) -> None:
        ...
        """
        with self.workspace.open_scaffold_file("repository_scaffold.json", "w") as f:
    ...
    def load_components(self) -> List:
        ...
        try:
            ...
            get_logger().warning(
                f"[scaffold] failed to load components: {e}; using empty"
            )
            return []
    ...
    def save_components(self, data: List) -> None:
        ...
        Args:
            data: List to save
        """
        with self.workspace.open_scaffold_file("high_level_components.json", "w") as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
    ...
    def get_ancestor_chain(self, relpath: str) -> List[Dict]:
        ...
        if relpath not in (".", "", "/"):
            ...
            nd = nodes.get(".", {})
            chain.append(
                {"path": ".", "summary": nd.get("summary"), "tags": nd.get("tags", [])}
            )

            # Build all ancestor paths (root to grandparent)
            for i in range(
                len(pieces) - 2
            ):  # Stop at grandparent (exclude current and parent)
    ...
    def merge_nodes(self, updates: Dict[str, NodeUpdate], step_id: int) -> None:
        ...
        for path, node in (updates or {}).items():
            try:
                ...
                get_logger().warning(
                    f"[scaffold] invalid NodeUpdate for {path}: {e}; skipping"
                )
                continue
            ...
            new_obj = {
                ...
                "languages": list(
                    dict.fromkeys([s.lower() for s in (model.languages or [])])
                ),
        ...
        if changed:
            ...
            get_logger().info(f"[scaffold] merged nodes: {changed}")
    ...
    def merge_components(self, comps: List[HighLevelComponent]) -> None:
        ...
        for comp in comps or []:
            try:
                model = (
                    comp
                    if isinstance(comp, HighLevelComponent)
                    else HighLevelComponent(**comp)
                )
            except Exception as e:
                get_logger().warning(
                    f"[scaffold] invalid component {getattr(comp, 'name', None)}: {e}; skipping"
                )
                continue
            ...
            if key in idx:
                ...
                # Union evidence
                seen = {
                    (e.get("type"), e.get("value")) for e in curr.get("evidence", [])
                }
                ...
                curr["confidence"] = max(
                    float(curr.get("confidence", 0.0)), float(model.confidence)
                )
                changed += 1
            else:
                # Add new component
        ...
        if changed:
            ...
            get_logger().info(f"[scaffold] merged components: {changed}")

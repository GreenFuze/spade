#!/usr/bin/env python3
"""
Test V7 Phase 1 with explore_repository_signals tool on jni_hello_world repository.
...
async def test_phase1_with_new_tool():
    ...
    # Test repository
    test_repo = project_root / "tests" / "test_repos" / "jni_hello_world"
    
    if not test_repo.exists():
        print(f"❌ Test repository not found: {test_repo}")
        return False
    
    print(f"🧪 Testing V7 Phase 1 with explore_repository_signals tool on jni_hello_world")
    ...
    try:
        ...
        # Accuracy analysis for JNI repository
        accuracy_score = 0
        total_checks = 5
        
        # Check primary language (should detect both C++ and Java)
        detected_languages = overview.get("primary_language", "")
        if "C++" in detected_languages or "Java" in detected_languages:
            print("✅ Correctly detected C++ or Java as primary language")
            accuracy_score += 1
        else:
            print(f"⚠️  Expected C++ or Java, got: {detected_languages}")
        
        # Check build system (should detect CMake)
        ...
        # Check directory structure (should have src/cpp and src/java)
        source_dirs = overview.get("directory_structure", {}).get("source_dirs", [])
        if "src" in source_dirs or "src/cpp" in source_dirs or "src/java" in source_dirs:
            print("✅ Correctly identified source directories")
            accuracy_score += 1
        else:
            print(f"⚠️  Did not identify source directories, got: {source_dirs}")
        ...
        # Check if it detected JNI (Java Native Interface)
        notes = overview.get("notes", "")
        if "jni" in notes.lower() or "java native" in notes.lower():
            print("✅ Correctly identified JNI components")
            accuracy_score += 1
        else:
            print("⚠️  Did not identify JNI components")

"""
Standalone test for V7 Phase 3: Artifact Discovery on jni_hello_world repository.

This test runs Phase 1, Phase 2, and Phase 3 in sequence to test the complete
artifact discovery pipeline on a multi-language JNI project.
"""
...
async def test_phase3_jni_hello_world():
    """Test Phase 3 artifact discovery on jni_hello_world repository."""
    
    # Setup logging
    logging.basicConfig(level=logging.INFO)
    logger = logging.getLogger("TestPhase3JNIHelloWorld")
    
    # Repository path
    repository_path = project_root / "tests" / "test_repos" / "jni_hello_world"
        ...
        if "artifact_discovery" not in phase3_result:
            ...
            if "build_system_artifacts" not in artifact_discovery:
                ...
                if "cmake" not in build_system_artifacts:
                    ...
                    # Check for libraries (JNI projects often have libraries)
                    ...
                    # Check for JVM artifacts (Java components)
                    if "jvm_artifacts" in cmake_artifacts:
                        jvm_artifacts = cmake_artifacts["jvm_artifacts"]
                        logger.info(f"✅ Found {len(jvm_artifacts)} JVM artifacts")
                        for jvm in jvm_artifacts:
                            logger.info(f"  - {jvm.get('name', 'unnamed')}")
            ...
            if "artifact_summary" not in artifact_discovery:
                ...
                if total_artifacts == 0:
                    ...
                    # Check for multi-language artifacts
                    executables = artifact_summary.get("executables", 0)
                    libraries = artifact_summary.get("libraries", 0)
                    jvm_artifacts = artifact_summary.get("jvm_artifacts", 0)
                    
                    logger.info(f"  - Executables: {executables}")
                    logger.info(f"  - Libraries: {libraries}")
                    logger.info(f"  - JVM Artifacts: {jvm_artifacts}")
            ...
            if "evidence_summary" not in artifact_discovery:
                ...
                if len(files_analyzed) == 0:
                    ...
                    # Check for JNI-specific patterns
                    patterns_found = evidence_summary.get("patterns_found", [])
                    if "jni" in str(patterns_found).lower():
                        logger.info("✅ JNI patterns detected")
                    else:
                        logger.info("ℹ️ No JNI patterns explicitly mentioned")
...
async def main():
    """Main test function."""
    success = await test_phase3_jni_hello_world()
    if success:

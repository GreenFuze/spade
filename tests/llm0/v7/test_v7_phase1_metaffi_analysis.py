...
async def test_phase1_with_new_tool():
    ...
    if not test_repo.exists():
        print(f"❌ MetaFFI repository not found: {test_repo}")
        return False
    
    print(f"🧪 Testing V7 Phase 1 with explore_repository_signals tool on MetaFFI")
    ...
    try:
        ...
        if "metaffi" in repo_name:
            print("✅ Correctly identified MetaFFI repository name")
            accuracy_score += 1
        else:
            print(f"⚠️  Expected MetaFFI, got: {overview.get('name')}")
        ...
        if "C++" in primary_lang or "multi" in primary_lang.lower():
            print("✅ Correctly detected C++ or multi-language as primary language")
            accuracy_score += 1
        else:
            print(f"⚠️  Expected C++ or multi-language, got: {primary_lang}")
        ...
        if len(source_dirs) >= 3:  # MetaFFI has multiple source directories
            print("✅ Correctly identified multiple source directories")
            accuracy_score += 1
        else:
            print(f"⚠️  Expected multiple source directories, got: {source_dirs}")
        ...
        if "framework" in repo_type.lower():
            print("✅ Correctly identified as framework")
            accuracy_score += 1
        else:
            print(f"⚠️  Expected framework, got: {repo_type}")

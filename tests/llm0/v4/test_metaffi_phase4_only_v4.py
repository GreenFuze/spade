#!/usr/bin/env python3
"""
Test V4 Phase 4 ONLY: Build System Analysis for MetaFFI
"""
...
async def test_phase4_only_metaffi():
    """Test Phase 4 ONLY: Build System Analysis for MetaFFI."""
    print("INFO:test_phase4_only_metaffi:Running Phase 4 ONLY: Build System Analysis...")
    ...
    print(f"INFO:test_phase4_only_metaffi:Using repository: {test_repo_path}")
    
    # Load Phase 1, Phase 2, and Phase 3 results from previous tests
    phase1_results = {
        "repository_overview": {
            "name": "MetaFFI",
            "type": "library",
    ...
    try:
        ...
        print(f"INFO:test_phase4_only_metaffi:Phase 4 completed in {execution_time:.1f} seconds")
        
        # Extract key information for logging
        build_targets = result.get("build_system", {}).get("build_targets", [])
        dependencies = result.get("build_system", {}).get("dependencies", [])
        
        print(f"INFO:test_phase4_only_metaffi:Build targets found: {len(build_targets)}")
        print(f"INFO:test_phase4_only_metaffi:Dependencies found: {len(dependencies)}")
        
        print("[OK] Phase 4 ONLY test with MetaFFI passed")
        return True
        
    except Exception as e:
        print(f"ERROR:test_phase4_only_metaffi:Phase 4 failed: {e}")
        import traceback
        print(f"Traceback: {traceback.format_exc()}")
        return False

if __name__ == "__main__":
    success = asyncio.run(test_phase4_only_metaffi())
    if not success:
        sys.exit(1)

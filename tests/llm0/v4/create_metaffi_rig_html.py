#!/usr/bin/env python3
"""
Create HTML graph for MetaFFI repository
"""
...
async def create_metaffi_rig_html():
    """Create RIG HTML graph for MetaFFI repository."""
    
    # Setup logging
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    logger = logging.getLogger("create_metaffi_rig_html")
    ...
    try:
        logger.info(f"Creating RIG HTML graph for MetaFFI...")
...
async def main():
    """Main function."""
    print("Creating RIG HTML graph for MetaFFI...")
    
    success = await create_metaffi_rig_html()

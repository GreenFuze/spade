#!/usr/bin/env python3
"""
Test V4 Phase 4 ONLY: Build System Analysis for cmake_hello_world
"""
...
async def test_phase4_only_cmake_hello_world():
    """Test Phase 4 ONLY: Build System Analysis for cmake_hello_world."""
    print("INFO:test_phase4_only_cmake_hello_world:Running Phase 4 ONLY: Build System Analysis...")
    ...
    print(f"INFO:test_phase4_only_cmake_hello_world:Using repository: {test_repo_path}")
    
    # Create mock Phase 1, Phase 2, and Phase 3 results for cmake_hello_world
    phase1_results = {
        "repository_overview": {
    ...
    try:
        ...
        print(f"INFO:test_phase4_only_cmake_hello_world:Phase 4 completed in {execution_time:.1f} seconds")
        
        # Extract key information for logging
        build_targets = result.get("build_system", {}).get("build_targets", [])
        dependencies = result.get("build_system", {}).get("dependencies", [])
        
        print(f"INFO:test_phase4_only_cmake_hello_world:Build targets found: {len(build_targets)}")
        print(f"INFO:test_phase4_only_cmake_hello_world:Dependencies found: {len(dependencies)}")
        
        print("[OK] Phase 4 ONLY test with cmake_hello_world passed")
        return True
        
    except Exception as e:
        print(f"ERROR:test_phase4_only_cmake_hello_world:Phase 4 failed: {e}")
        import traceback
        print(f"Traceback: {traceback.format_exc()}")
        return False

if __name__ == "__main__":
    success = asyncio.run(test_phase4_only_cmake_hello_world())
    if not success:
        sys.exit(1)

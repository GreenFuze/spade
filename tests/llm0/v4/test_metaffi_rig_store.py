#!/usr/bin/env python3
"""
Test RIG Store functionality with MetaFFI repository
...
async def test_metaffi_rig_store():
    """Test RIG store functionality with MetaFFI repository."""
    
    # Setup logging
    logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    logger = logging.getLogger("test_metaffi_rig_store")
    ...
    try:
        logger.info(f"Testing RIG Store with MetaFFI repository...")
        ...
        db_path = Path("metaffi_rig_store.db")
        store = RIGStore(db_path)
        
        rig_id = store.save_rig(original_rig, "MetaFFI V4+ Enhanced RIG")
...
async def main():
    """Main function."""
    print("Testing RIG Store functionality with MetaFFI...")
    print("This will generate a RIG, save it to SQLite, load it back, and compare")
    
    success = await test_metaffi_rig_store()

...
from models import NodeUpdate, HighLevelComponent, Evidence
...
def test_initialization():
    ...
    if nodes == {} and components == []:
        ...
        print(f"✗ Expected empty dict and list, got {type(nodes)} and {type(components)}")
        return False
...
def test_merge_components():
    ...
    api_component = next((c for c in stored_components if c.get("name") == "API Layer"), None)
...
def test_component_deduplication():
    ...
    if len(final_components) == 1:
        ...
        print(f"✗ Expected 1 component after deduplication, got {len(final_components)}")
        return False
    
    # Check that the component was updated correctly
    updated_component = final_components[0]
    if (updated_component.get("role") == "REST API Service" and  # Latest role
        updated_component.get("confidence") == 0.9 and  # Max confidence
        len(updated_component.get("evidence", [])) == 2):  # Union of evidence
...
def test_invalid_data_handling():
    ...
    invalid_updates = {
        "invalid": {"invalid": "data"}  # Not a NodeUpdate
    }
    ...
    invalid_components = [
        {"invalid": "component"}  # Not a HighLevelComponent
    ]
...
def test_acceptance_criteria():
    ...
    if (store.nodes_path.exists() and 
        store.components_path.exists() and
        store.load_nodes() == {} and
    ...
    if ("test" in nodes and 
        nodes["test"]["last_updated_step"] == 5 and
        nodes["test"]["summary"] == "Test node"):
    ...
    components = store.load_components()
    if (len(components) == 1 and  # Deduplicated
        components[0]["role"] == "Updated Role" and  # Latest role
        components[0]["confidence"] == 0.9 and  # Max confidence
        len(components[0]["evidence"]) == 2):  # Union of evidence

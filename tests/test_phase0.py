...
from schemas import RunConfig, Worklist
...
def test_worklist_store():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if wl.queue != ["."] or wl.visited != []:
            print(
                f"✗ Expected initial state queue=['.'], visited=[], got {wl.queue}, {wl.visited}"
            )
            return False
        ...
        worklist.mark_visited("test")
        if not worklist.is_visited("test"):
            print("✗ Expected test to be marked as visited")
            return False

        if worklist.is_visited("other"):
            print("✗ Expected other to not be visited")
            return False
...
def test_phase0_with_dummy_transport():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Create minimal run_phase0.yaml
        config = RunConfig(
        ...
        (repo_root / ".spade" / "run_phase0.yaml").write_text(
        ...
        (repo_root / ".spade" / "snapshot" / "dirmeta.json").write_text(
            json.dumps(root_dirmeta)
        )
        ...
        for child in ["src", "docs", "tests"]:
            ...
            (repo_root / ".spade" / "snapshot" / child / "dirmeta.json").write_text(
                json.dumps(child_dirmeta)
            )
...
def test_phase0_resume():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        (repo_root / ".spade" / "worklist.json").write_text(
            json.dumps(existing_worklist)
        )
        ...
        try:
            ...
            wl = worklist.load()
            print(
                f"✓ Final worklist state: queue={len(wl.queue)}, visited={len(wl.visited)}"
            )

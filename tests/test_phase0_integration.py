...
def test_phase0_with_real_fakeapp():
    ...
    required_files = [
        repo_root / ".spade" / "run_phase0.yaml",
    ...
    try:
        ...
        config_path = repo_root / ".spade" / "run_phase0.yaml"
        config = RunConfig(**yaml.safe_load(config_path.read_text(encoding="utf-8")))
        ...
        if telemetry_lines:
            ...
            for i, line in enumerate(telemetry_lines[:3]):  # Show first 3 lines
                ...
                print(
                    f"  Step {data['step']}: {data['path']} (d={data['depth']}, valid={data['json_valid']})"
                )
